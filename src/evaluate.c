...
const int PawnValue = S( 114, 121);
const int KnightValue = S( 517, 381);
const int BishopValue = S( 537, 392);
const int RookValue = S( 735, 692);
const int QueenValue = S(1378,1384);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 114, 121}, { 517, 381}, { 537, 392}, { 735, 692},
    {1378,1384}, {   0,   0}, {   0,   0}, {   0,   0},
};

/* Pawn Evaluation Terms */

const int PawnIsolated = S( -11,  -2);

const int PawnStacked = S(  -7, -29);
...
const int PawnConnected32[32] = {
        ...
        S(   0, -16), S(   8,   6), S(   8, -13), S(   7,  17),
        S(   9,   1), S(  29,   0), S(  21,   8), S(  23,  18),
        S(  10,   3), S(  22,   6), S(  10,  11), S(  14,  25),
        S(  16,   8), S(  19,  21), S(  29,  23), S(  36,  25),
        S(  19,  50), S(  21,  61), S(  60,  62), S(  51,  72),
        S( 105, -15), S( 198,  15), S( 227,  21), S( 249,  74),
        S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightBehindPawn = S(   5,  21);

const int KnightMobility[9] = {
        S( -86, -85), S( -29, -94), S( -13, -36), S(   2, -21),
        S(  13, -18), S(  20,  -5), S(  29,  -9), S(  42, -14),
        S(  54, -37),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  36,  73);

const int BishopRammedPawns = S( -10, -10);
...
const int BishopMobility[14] = {
        S( -55,-127), S( -31, -71), S(  -7, -52), S(   2, -23),
        S(  15, -17), S(  25,  -4), S(  28,   2), S(  30,   4),
        S(  33,   5), S(  36,   3), S(  39,   7), S(  47, -16),
        S(  47,  -4), S(  43, -37),
};
...
const int RookOnSeventh = S(  -9,  30);

const int RookMobility[15] = {
        S(-144,-107), S( -49,-116), S(  -4, -68), S(   1, -20),
        S(   1,  -5), S(   1,  11), S(   2,  19), S(   6,  26),
        S(  13,  25), S(  15,  35), S(  20,  36), S(  24,  42),
        S(  27,  44), S(  28,  42), S(  18,  49),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
        S( -61,-263), S(-217,-390), S( -47,-205), S( -26,-190),
        S(  -8,-132), S( -11, -70), S(  -4, -88), S(  -8, -65),
        S(  -2, -61), S(  -1, -45), S(   2, -34), S(   1, -18),
        S(   2, -14), S(   5,   0), S(   5,   9), S(   4,   6),
        S(   2,  23), S(   1,  23), S(   4,  21), S(   0,  20),
        S(  -4,  19), S(  17,  19), S(   7,   0), S(  27,  -1),
        S(  31,   7), S(  55,  -9), S( -50, -18), S(   0,  -2),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
        S( -24,   0), S( -12,   6), S(  -1,   3), S(   8,   0),
        S(  20,  -9), S(  30,  -1), S(  31,  13), S(  23,   0),
        S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][8][8] = {
        {{S( -12,   9), S(  11, -17), S(  12,  -4), S(  21,   0), S(   6,   6), S(  30,   3), S(  -1, -33), S( -30,  13)},
                {S(  10,   7), S(  15, -12), S(   8, -11), S(  -2, -14), S( -26,   2), S( -65,  78), S( 100,  93), S( -30,  10)},
                {S(  21,  11), S(   7,  -8), S( -21,   4), S(  -7,  -8), S( -25,  -4), S(  15,  -6), S(   4,  65), S( -19,   5)},
                {S(   9,  28), S(  10,   0), S(  -7,  -2), S(  18, -21), S(  19, -43), S( -24, -30), S(-146,  41), S(  -1,   0)},
                {S( -13,  17), S(  -6,   2), S( -28,   3), S( -14,   7), S( -22, -12), S( -44, -19), S(  38,  -8), S( -13,   1)},
                {S(  22,   4), S(  18, -10), S( -26,   3), S( -10, -20), S(   6, -36), S(  15, -49), S(  49, -34), S( -18,   2)},
                {S(  19,   6), S(   1, -13), S( -28,  -1), S( -20, -18), S( -28, -23), S( -43,   9), S(   6,  46), S(  -4,  10)},
                {S( -14,  -3), S(   1, -13), S(   5,   0), S(   1,   1), S( -24,  14), S(  -9,  28), S(-201,  78), S( -15,  21)}},
        {{S(   0,   0), S(   3, -12), S(   8, -12), S( -44,  17), S(   0,  -9), S(   9,  50), S(-171, -15), S( -44,  19)},
                {S(   0,   0), S(  19, -13), S(  12, -10), S(  -6, -11), S(   9, -34), S(  51, 102), S(-187, -10), S( -47,   9)},
                {S(   0,   0), S(  26,  -3), S(   0,  -4), S(  16, -27), S(  14,  -9), S(-114,  44), S( -89, -83), S( -26,  -3)},
                {S(   0,   0), S(  -3,   7), S(  -9,  15), S( -15,   7), S( -25,   1), S(-116,  10), S(   4, -50), S( -28,   0)},
                {S(   0,   0), S(   6,   3), S(   7,   0), S(  22,  -3), S(  10, -29), S( -46,   5), S(-109, -72), S(  -2,  -2)},
                {S(   0,   0), S(   6,   1), S( -23,   4), S( -12, -17), S(  31, -41), S( -44,   6), S(  59,  46), S( -23,   2)},
                {S(   0,   0), S(  12,  -4), S(  -3,  -4), S( -26,  -5), S( -30, -15), S(  19,  12), S( -59, -65), S( -40,  21)},
                {S(   0,   0), S(   0, -23), S(  16, -20), S( -27,  -3), S( -28,  -3), S(   6, -16), S(-240, -73), S( -20,  20)}},
};
...
const int PassedPawn[2][2][8] = {
        {{S(   0,   0), S( -36, -27), S( -24,   5), S( -20,  12), S(  13,   8), S(  47,   7), S( 141,  41), S(   0,   0)},
                {S(   0,   0), S( -19, -24), S( -22,   1), S( -14,  24), S(   5,  44), S(  60,  75), S( 185, 129), S(   0,   0)}},
        {{S(   0,   0), S( -20,  -9), S( -32,   6), S( -14,  27), S(  16,  44), S(  51,  73), S( 212, 163), S(   0,   0)},
                {S(   0,   0), S( -32, -16), S( -23,  -4), S( -13,  34), S(  15,  77), S(  37, 167), S( 122, 347), S(   0,   0)}},
};
...
const int ThreatWeakPawn = S( -24, -38);
const int ThreatMinorAttackedByPawn = S( -72, -55);
const int ThreatMinorAttackedByMajor = S( -43, -56);
const int ThreatRookAttackedByLesser = S( -68, -24);
const int ThreatQueenAttackedByOne = S( -72,  -2);
const int ThreatOverloadedPieces = S( -12, -23);
const int ThreatByPawnPush = S(  19,  18);
